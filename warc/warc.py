...
class WARCReader:
    ...
    def read_header(self, fileobj):
        version_line = fileobj.readline().decode('UTF-8',errors='ignore')
        if not version_line:
            return None
        #print(version_line)
       ...
       #     raise IOError("Bad version line: %r" % version_line)
        ...
        headers = {}
        while True:
            line = fileobj.readline().decode('UTF-8',errors='ignore')
    ...
    def read_record(self):
        ...
        self.current_payload = self.read_payload(fileobj, header.content_length)
